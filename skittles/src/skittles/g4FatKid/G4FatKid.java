...
public class G4FatKid extends Player {
		...
		if (!isTasteKnowledgeSufficient()) {
			...
			for (int i = 0; i < numberOfColors; i++) {
				...
				if (mostDesired == i && skittleBalanceArray[i] > 0) {
					for (int j = 0; j < numberOfColors; j++) {
						if (j != mostDesired && skittleBalanceArray[j] > 0) {
							...
							//aintDesire[i] = Math.min(skittleBalanceArray[j], 3);
				...
				if (mostUndesired == i && skittleBalanceArray[i] > 0) {
					for (int j = 0; j < numberOfColors; j++) {
						if (j != mostDesired && skittleBalanceArray[j] > 0) {
							aintOffer[i] = skittleBalanceArray[j];
				...
				if (mostDesired == i && skittleBalanceArray[i] > 0) {
					for (int j = 0; j < numberOfColors; j++) {
						if (mostUndesired == j && skittleBalanceArray[j] > 0) {
							...
							{aintDesire[i] = skittleBalanceArray[mostUndesired] / 3;
	...
	@Override
	public void initialize(int playerNum, double dblTasteMean,int playerIndex, String className,
			int[] aintInHand) {
